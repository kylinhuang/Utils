 ...
 * 后台进程.确保进入后台也在运行
 */
public class RemindService extends Service implements Runnable, MediaPlayer.OnCompletionListener {
    ...
    public void onCreate() {
        ...
        wakeLock = pm.newWakeLock(PowerManager.PARTIAL_WAKE_LOCK, RemindService.class.getName());
        if(wakeLock!=null){
            wakeLock.acquire();
        }else{
...
//            RequestUtils.getRequest(AppConst.authUrl("person/active/app?version="+getApplicationContext().getApplicationInfo()),handler);
    ...
    public void onDestroy() {
        ...
        localIntent.setClass(this, RemindService.class);
